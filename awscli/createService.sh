...
then
	...
	aws ecs create-service --cluster bwce-cluster --service-name ShoppingCart --task-definition ShoppingCart:$TASK_REV --desired-count 2 --launch-type "FARGATE" --deployment-configuration "maximumPercent=200,minimumHealthyPercent=50"  --network-configuration "awsvpcConfiguration={subnets=[subnet-a55894ed],securityGroups=[sg-9f44cae5],assignPublicIp=ENABLED}"
